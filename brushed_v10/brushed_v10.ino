...
float wc = 10.0;      // Controller Bandwidth
float wo = 70.0;      // Observer Bandwidth
float b0 = 100.0;     // System Gain
const float dt = 0.01; // Sample time (10ms)
...
void apply_motor_pwm(float control_u) {
  int power = constrain(abs(control_u), 0, 128); 

  if (control_u > 0) {
    digitalWrite(PIN_MOTOR_A, LOW);
    analogWrite(PIN_MOTOR_B, power);
  } else {
